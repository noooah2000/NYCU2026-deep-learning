...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, label.shape[0]):
            ...
            curr_label = label[i].to(self.args.device)

            F_prev = self.frame_transformation(prev_frame)
            L_curr = self.label_transformation(curr_label)
            
            z = torch.randn(prev_frame.size(0), self.args.N_dim, F_prev.size(2), F_prev.size(3)).to(self.args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path',     type=str,    default="./checkpoints/best_model.ckpt",help="The path of your checkpoints")   
